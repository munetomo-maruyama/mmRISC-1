...
// Rev.04 2025.09.19 M.Maruyama Adapted Generic Debugger I/F
//-----------------------------------------------------------
...
//
//-------------------------------------------------------------------
// [BUG] Can not connect cJTAG I/F of some commercial IDEs
// Can not connect cJTAG if illegal activation sequence (TCKC toggle)
// occurs just after online sequence. After the illegal activation,
// online_clr remains high, and it prevents online active at the
// next online sequence because online_clr_res is still active
// when escape_set is asserted.
//-------------------------------------------------------------------
//assign online_clr_res = RES | escape;
//-------------------------------------------------------------------
// [FIX] Can connect cJTAG I/F of some commercial IDEs
// Can connect if Illegal Activation just after Online Sequence.
// This method uses asynchronous reset of D-F/F that is generated by
// combinational logic which may generate glitches, 
// but in this case, there are no problem.
//-------------------------------------------------------------------
assign online_clr_res = RES | escape_set;
//-------------------------------------------------------------------
assign online_clr_set = online & ~actv_count_equ & ~cjtag_in_oscan1;
//
//-------------------------------------------------------------------
// [BUG] Some commerial IDEs generate 1-cycle Illegal Activation
// Sequence after Online Sequence. Such Illegal Sequence should
// clear the online state, but the bug can not do that. 
//-------------------------------------------------------------------
//always @(posedge TCKC, posedge online_clr_res)
//begin
//    if (online_clr_res)
//-------------------------------------------------------------------
// [FIX] Can connect cJTAG I/F of the commercial IDEs.
// Can connect if Illegal Activation just after Online Sequence.
// Clear online_clr by neg level of TMSC_I.
//-------------------------------------------------------------------
always @(posedge TCKC, negedge TMSC_I)
begin
    if (~TMSC_I)
        online_clr <= 1'b0;
