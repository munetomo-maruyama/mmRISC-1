...
//-------------------------------------------
// Exception Handler for intentional access
// to impure pointer in "srand()"
//-------------------------------------------
__attribute__ ((interrupt)) void EXCEPTION_Handler(void)
{
    ...
    if ((cause == 4) || (cause == 6))
    {
        // skip the instruction which invokes this exception
        addr = read_csr(MEPC);
        code = *((uint8_t*)addr);
        size = ((code & 0x03) == 0x03)? 4 : 2;
        addr = addr + size;
        write_csr(MEPC, addr);
    }
}
